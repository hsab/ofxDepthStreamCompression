...
void ofxDepthCompressedFrame::setRegistration(float pixel_size, float distance) {
   compressed[3] = boost::algorithm::clamp((short)(pixel_size * 10000), numeric_limits<short>::min(), numeric_limits<short>::max());
   compressed[4] = boost::algorithm::clamp((short)(distance * 10), numeric_limits<short>::min(), numeric_limits<short>::max());
}
   ...
   if (compressedDirty) {
      ...
      if (!isKeyFrame()) {
         ...
         for (int i = 0; i < pixels.size(); i++) {
            int pos = i - lastPos;
            if (pixels[i] == 0 && pos < std::numeric_limits<unsigned short>::max())
               continue;
